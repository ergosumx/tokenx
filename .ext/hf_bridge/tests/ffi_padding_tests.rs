...
use tokenx_bridge::ffi::padding::{tokenizers_disable_padding, tokenizers_enable_padding};
...
fn tokenizers_disable_padding_clears_configuration() {
    ...
    let mut status = -1;
    let result = unsafe {
        tokenizers_disable_padding(&mut tokenizer as *mut CTokenizer, ptr::addr_of_mut!(status))
    };

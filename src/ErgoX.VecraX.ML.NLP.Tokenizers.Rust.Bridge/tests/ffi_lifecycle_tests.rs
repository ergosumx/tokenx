use std::ffi::CString;
use tokenx_bridge::ffi::lifecycle::{tokenizers_create, tokenizers_free, tokenizers_free_string};
...
fn tokenizers_create_builds_tokenizer_from_json() {
    ...
    let mut status = -1;
    let tokenizer = unsafe { tokenizers_create(json.as_ptr(), &mut status) };
...
fn tokenizers_create_reports_invalid_pointer() {
    let mut status = -1;
    let tokenizer = unsafe { tokenizers_create(std::ptr::null(), &mut status) };
    assert!(tokenizer.is_null());
    assert_ne!(status, 0);
}

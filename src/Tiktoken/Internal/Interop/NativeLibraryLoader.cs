...
{
    private const string LibraryLogicalName = "tt_bridge";
    ...
    {
        NativeLibraryResolverRegistry.Register(typeof(NativeLibraryLoader).Assembly, Resolve);
    }

    private static IntPtr Resolve(string libraryName, Assembly assembly, DllImportSearchPath? searchPath)
    {
        if (!string.Equals(libraryName, LibraryLogicalName, StringComparison.Ordinal))
        {
            return IntPtr.Zero;
        }
    ...
    private static IEnumerable<string> EnumerateCandidatePaths(string rid, string fileName, Assembly assembly)
    {
        ...
        var runtimeRelativePath = Path.Combine("runtimes", rid, "native", fileName);
